...
class CityParser:
	def __init__(self, company_name):
		...
		stream = rsp.read()
		tmp_file = open(dirname(os.path.realpath(__file__)) + '/resources/tmp1.txt', 'w')
		tmp_file.write(stream)
		rsp.close()
		tmp_file.close()
		print("resources/tmp.txt")
		soup = bs4.BeautifulSoup(open(dirname(os.path.realpath(__file__)) + '/resources/tmp1.txt', 'r'))
		try:
