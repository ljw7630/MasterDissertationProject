...
class DBHelper():
	...
	def getNotExistFileNames(limit=1000):
		res = DBHelper.getCursor().execute("select file_name from survey_data where file_exists=0 and limit=%i" % limit)
		return res.fetchall()

	@staticmethod
	def getNotRDFedFileName(limit=1000):
		res = DBHelper.getCursor().execute("select file_name from survey_data where file_exists=1 and rdf=0 and limit=%i" % limit)
		return res.fetchall()
